...
function doRectanglesOverlap(rect1, rect2) {
  if (
    ...
    && ((rect1.top < rect2.top && rect2.top < rect1.top + rect1.height)
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < splitedStr.length; i++) {
    const charCount = splitedStr.reduce((acc, el) => {
      // eslint-disable-next-line no-param-reassign
      if (splitedStr[i] === el) acc++;
      return acc;
    }, 0);

    if (charCount === 1) return splitedStr[i];
  }
...
function isCreditCardNumber(ccn) {
  const digits = `${ccn}`.split('').map((val) => parseInt(val, 10));
...
function getDigitalRoot(num) {
  ...
  const digits = `${num}`.split('').map((val) => parseInt(val, 10));
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < shortestPath.length; i++) {
    const toCompare = segments.map((val) => val[i]);
    if (toCompare.every((val) => val === shortestPath[i])) {
      common.push(shortestPath[i]);
    } else {
      break;
    }
  }
